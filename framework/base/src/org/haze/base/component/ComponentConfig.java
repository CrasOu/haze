 ...
 * under the License.
 *******************************************************************************/
package org.haze.base.component;
...
import org.haze.base.location.FlexibleLocation;
import org.haze.base.util.Debug;
import org.haze.base.util.KeyStoreUtil;
import org.haze.base.util.UtilURL;
import org.haze.base.util.UtilValidate;
import org.haze.base.util.UtilXml;
import org.w3c.dom.Document;
import org.w3c.dom.Element;
import org.xml.sax.SAXException;

/**
 * ComponentConfig - Component configuration class for haze-container.xml
 *
 */
public class ComponentConfig {

    public static final String module = ComponentConfig.class.getName();
    public static final String HAZE_COMPONENT_XML_FILENAME = "haze-component.xml";
                ...
                synchronized (ComponentConfig.class) {
                    ...
                    if (componentConfig == null) {
                        ...
                        if (componentConfigs.containsKey(componentConfig.getGlobalName())) {
                            Debug.logWarning("WARNING: Loading haze-component using a global name that already exists, will over-write: " + componentConfig.getGlobalName(), module);
                        }
    ...
    protected ComponentConfig(String globalName, String rootLocation) throws ComponentException {
        ...
        String xmlFilename = rootLocation + "/" + HAZE_COMPONENT_XML_FILENAME;
        URL xmlUrl = UtilURL.fromFilename(xmlFilename);
        if (xmlUrl == null) {
            throw new ComponentException("Could not find the " + HAZE_COMPONENT_XML_FILENAME + " configuration file in the component root location: " + rootLocation);
        }

        Document hazeComponentDocument = null;
        try {
        	hazeComponentDocument = UtilXml.readXmlDocument(xmlUrl, true);
        ...
        Element hazeComponentElement = hazeComponentDocument.getDocumentElement();
        this.componentName = hazeComponentElement.getAttribute("name");
        this.enabled = "true".equalsIgnoreCase(hazeComponentElement.getAttribute("enabled"));
        if (UtilValidate.isEmpty(this.globalName)) {
            this.globalName = this.componentName;
        }

        // resource-loader - resourceLoaderInfos
        for (Element curElement: UtilXml.childElementList(hazeComponentElement, "resource-loader")) {
        ...
        for (Element curElement: UtilXml.childElementList(hazeComponentElement, "classpath")) {
        ...
        for (Element curElement: UtilXml.childElementList(hazeComponentElement, "entity-resource")) {
        ...
        for (Element curElement: UtilXml.childElementList(hazeComponentElement, "service-resource")) {
        ...
        for (Element curElement: UtilXml.childElementList(hazeComponentElement, "test-suite")) {
        ...
        for (Element curElement: UtilXml.childElementList(hazeComponentElement, "keystore")) {
            KeystoreInfo keystoreInfo = new KeystoreInfo(this, curElement);
            this.keystoreInfos.add(keystoreInfo);
        }

        // webapp - webappInfos
        for (Element curElement: UtilXml.childElementList(hazeComponentElement, "webapp")) {
