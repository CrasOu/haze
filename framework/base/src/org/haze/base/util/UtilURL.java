...
public class UtilURL {
    ...
    public static URL fromResource(String resourceName, ClassLoader loader) {
        ...
        if (url == null) url = fromHazeHomePath(resourceName);
    ...
    public static URL fromHazeHomePath(String filename) {
        String hazeHome = System.getProperty("haze.home");
        if (hazeHome == null) {
            Debug.logWarning("No haze.home property set in environment", module);
            return null;
        }
        String newFilename = hazeHome;
    ...
    public static String getHazeHomeRelativeLocation(URL fileUrl) {
        String hazeHome = System.getProperty("haze.home");
        String path = fileUrl.getPath();
        if (path.startsWith(hazeHome)) {
            // note: the +1 is to remove the leading slash
            path = path.substring(hazeHome.length()+1);
        }
        return path;
    }
