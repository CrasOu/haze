...
rem set HAZE_LOG=runtime\logs\console.log
...
rem ### Different ways of launching Haze ##############################
rem ### start haze with previous set VMARGS
rem "%JAVA_HOME%\bin\java" %VMARGS% -jar haze.jar > %HAZE_LOG%

rem ### This one is for more of a debugging mode
rem "%JAVA_HOME%\bin\java" -Xms128M -Xmx512M -XX:MaxPermSize=512m -Xdebug -Xnoagent -Djava.compiler=NONE -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=5005 -jar haze.jar > runtime\logs\console.log

rem ### Simple easy to read line
cd %HAZE_HOME%
echo on
"%JAVA_HOME%\bin\java" -Xms128M -Xmx512M -XX:MaxPermSize=512m -jar haze.jar -both
echo off
rem ### If you would prefer the console output to be logged rather than displayed switch out the above line for this one
rem "%JAVA_HOME%\bin\java" -Xms128M -Xmx512M -XX:MaxPermSize=512m -jar haze.jar > runtime\logs\console.log
