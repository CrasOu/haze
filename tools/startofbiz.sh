...
# set the parent directory as Haze Home
HAZE_HOME="$( cd -P "$( dirname "$0" )" && pwd )"/..

# console log file
HAZE_LOG=runtime/logs/console.log
...
# start haze
#$JAVA $VMARGS -jar haze.jar $* >>$HAZE_LOG 2>>$HAZE_LOG&
(cd "$HAZE_HOME" && exec "$JAVA" $VMARGS -jar haze.jar "$@")
